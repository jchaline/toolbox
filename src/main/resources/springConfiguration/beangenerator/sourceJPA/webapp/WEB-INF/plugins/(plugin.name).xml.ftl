<?xml version="1.0" encoding="ISO-8859-1" standalone="no"?><plug-in>
    ...
    <class>fr.paris.lutece.plugins.${(plugin.name)!'pluginName'}.service.${(plugin.name?cap_first)!'pluginName'}Plugin</class>
    ...
    <admin-features>
    	<#list plugin.beanList as beanItem>
    		<admin-feature>
				...
				<feature-title>${(plugin.name)!}.adminFeature.${(plugin.name)!}.${beanItem.name}Management.name</feature-title>
				<feature-description>${(plugin.name)!}.adminFeature.${(plugin.name)!}.${beanItem.name}Management.description</feature-description>
